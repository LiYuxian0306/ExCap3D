...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, mask_type):
        ...
        for batch_id, (map_id, target_id) in enumerate(indices):
            ...
            # keep only the required points/segments
            map = map[:, point_idx]
    ...
    def forward(self, outputs, targets, mask_type):
        ...
        """
        # 'pred_logits', 'pred_masks', 'sampled_coords', 'backbone_features', 'final_queries'
        outputs_without_aux = {
            k: v for k, v in outputs.items() if k != "aux_outputs"
        }

        # Retrieve the matching between the outputs of the last layer and the targets
        # len(indices)=batch_size, indices[i] = (src_idx [1,2,3], tgt_idx [4,5,6])
