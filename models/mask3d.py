...
from models.modules.common import conv, conv_tr_gen, conv_tr, ConvType
...
class Mask3D(nn.Module):
    def __init__(
        ...
        elif self.pos_enc_type == "fourier": # default
        ...
        for _ in range(num_shared):
            ...
            # levels of the 3d features to do cross attn on
            # default: [0,1,2,3]
            for _, hlevel in enumerate(self.hlevels):
        ...
        if self.upconv_seg_preds:
            self.upconv_seg_layer = conv_tr_gen(
    ...
    def forward(
        ...
        # pass through backbone, get features
        # feats: N,96    aux: list of 5 elems [(n1,256), (n2,256), (n3, 128), (n4, 96), (N,96)]
        # n1 = smallest, coarse. N = largest (voxels), finest
        # x -> sparsetensor with coordinates, features (colors, normals, coordinates)
        ...
        with torch.no_grad():
            ...
            # add the raw coordinates to the outputs? 
            coords = [coordinates]
        ...
        if self.train_on_segments: # default true
            mask_segments = []
        ...
        # get queries and query_pos 
        if self.non_parametric_queries: # default true
            # use this by default
            # list[nqueries, nqueries..]
            fps_idx = [
            ...
            # sampled_coords = b,nqueries,3 -> query_pos = B,128,nqueries
            query_pos = self.pos_enc(
                sampled_coords.float(), input_range=[mins, maxs]
            )  # Batch, Dim, queries
            query_pos = self.query_projection(query_pos) # B,128,nqueries (conv1d on channel dim, kernel=1,stride=1)

            if not self.use_np_features: # use np features default false ->
        ...
        # default: 3
        # go over each decoder
        for decoder_counter in range(self.num_decoders):
            if self.shared_decoder: #default: true
                # always using same decoder
                decoder_counter = 0
            # default: [0,1,2,3] = 4 coarsest levels in the feature backbone (see paper)
            # cross attn on different levels of 3d features
            for i, hlevel in enumerate(self.hlevels):
                # mask module: predict masks and semantics per mask
                ...
                # rest is query refinement? = masked cross attn + self attn, get new set of queries from existing ones
                decomposed_aux = aux[hlevel].decomposed_features
                # [(Npoints, nqueries)] for different samples
                decomposed_attn = attn_mask.decomposed_features

                # max of num points at this level across samples
                ...
                # sample voxels to have same number of voxel (features) in each sample ("batch entry" in paper)
                # pad + mask attention accordingly
                # len=1 = 1 pc from each 3d feature level?
                ...
                # use the indices to pick points in each sample
                # features
                ...
                # query refinement -> output=100,bsize,128
                ...
                pred_attn_masks.append(decomposed_attn)

        # last mask module to get final outputs
        if self.train_on_segments:
            # class: bsize, 100, nclasses
            # mask: list of (nsegments, nqueries=100) for each sample
            # get attn_mask for viz here
            output_class, outputs_mask, attn_mask = self.mask_module(
                queries,
                mask_features,
                mask_segments,
        ...
        predictions_class.append(output_class)
        predictions_mask.append(outputs_mask)
        pred_attn_masks.append(attn_mask.decomposed_features)

        # upsample segment masks to each voxel if specified using minkowskiconvolution
        if self.upconv_seg_preds:
            # get the locations of segments (centers) and logits on the centers
            # upconv seg preds: list of (nsegments, nqueries=100) for each sample
            # predictions_mask[-1] is a list of nsegments, nqueries for each sample
            bsize = len(predictions_mask[-1])
            logit_outputs = []
            for bid in range(batch_size):
                # nseg, nqueries
                seg_mask_logits = predictions_mask[-1][bid]
                # get raw coords for this sample
                sample_coords = coordinates.decomposed_features[bid]

                seg_centroids = scatter_mean(sample_coords, point2segment[bid], dim=0) # nsegments, 3                
                input_stensor = ME.SparseTensor(
                    ...
                    coordinates=ME.utils.batched_coordinates([seg_centroids / 0.02]).to(seg_mask_logits.device), 
                    # aggregate logits again on voxels
                    quantization_mode=ME.SparseTensorQuantizationMode.UNWEIGHTED_AVERAGE 
                )

                output_coords = (sample_coords // 0.02).long()
                breakpoint()
                upsampled_stensor = self.upconv_seg_layer(input_stensor, output_coords)

                # TODO: add back into batch


        # query pos: 100, bsize, 128
        return {
            ...
            "sampled_coords": sampled_coords.detach().cpu().numpy() # B,100,3
            if sampled_coords is not None
            else None,
            "backbone_features": pcd_features,
            "final_queries": queries, #bsize, nqueries, 128
    ...
    def mask_module(
        ...
        if ret_attn_mask:
            ...
            attn_mask = me.SparseTensor(
                # make attn mask binary, sparse tensor with coordinates
